...
trait Logging {
  private val _logger: LazyConstant[Logger] = LazyConstant.of(() => LoggerFactory.getLogger(getClass))
  ...
  protected object logger {
    ...
    def warn(message: => String, throwable: Throwable): Unit = _logger.get().warn(message, throwable)
    def error(message: => String): Unit = _logger.get().error(message)
    def error(message: => String, throwable: Throwable): Unit = _logger.get().error(message, throwable)
  }
